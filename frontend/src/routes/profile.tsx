...
function Profile() {
  const { leaderboard, badges } = Route.useLoaderData();
  const { user } = useAuth();
  ...
  return (
    <AppShell>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-10">
        {badges.map((b: any) => (
          <div key={b.id} className={`rounded-2xl ring-1 ring-border p-4 flex items-center gap-3`}>
            <div className="size-12 rounded-xl bg-brand-light grid place-items-center text-2xl" title={b.name}>{b.icon}</div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium">{b.name}</p>
              <p className="text-xs text-muted-foreground line-clamp-2">Earned on {new Date(b.date).toLocaleDateString()}</p>
            </div>
      ...
      <div className="rounded-2xl ring-1 ring-border bg-card overflow-hidden">
        {leaderboard.map((e: any, i: number) => (
          <div key={e.rank} className={`flex items-center gap-4 px-5 py-3 ${i > 0 ? "border-t border-border" : ""} ${e.isYou ? "bg-brand/5" : ""}`}>
            <span className="text-sm font-medium text-muted-foreground w-8">#{e.rank}</span>
            <div className={`size-9 rounded-full grid place-items-center text-xs font-semibold ${e.isYou ? "bg-brand/15 text-brand" : "bg-muted text-muted-foreground"}`}>{e.initials}</div>
