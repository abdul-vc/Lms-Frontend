...
function ProfileTab() {
  ...
  const handleSaveProfile = async () => {
    ...
    setProfileSuccess('');
    try {
      const res = await authFetch(`${API_URL}/users/${data.id}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Update your account identity and email address.</p>
        </div>
          ...
          <div className="bg-card rounded-3xl max-w-md w-full p-6 text-foreground shadow-2xl border border-border relative">
            ...
            <form onSubmit={handleCreateSuperAdmin} className="space-y-3.5 text-xs">
              ...
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-[11px] text-emerald-300 flex items-start gap-2 mt-2">
                ...
                <p>
                  Upon creation, an official welcome email containing the <strong>Login Portal URL ({window.location.origin}/login)</strong>, username, and password will be sent to the recipient's email address.
                </p>
              </div>
